        ...
        cgFormBuilder(container, {
            ...
            sections: [
                ...
                },
                {
                    ...
                    showFor: ['static'],
                    fields: [
                        { name: 'static_host', why: "Static NAT <b>çift yönlüdür</b>: bu host artık dışarıdan da erişilebilir hale gelir. Outside arayüze sınırlayıcı bir ACL bağlanmazsa sunucu tüm internete açılmış olur.", label: 'İç Host', type: 'text', validate: 'ip', optional: true, placeholder: '192.168.1.10', hint: 'NAT edilecek iç IP adresi' },
                        { name: 'static_mapped', why: "Dışarıya görünecek adres; ASA bu IP için otomatik proxy-ARP yapar. Adres outside subnetinin dışındaysa ISP’nin bu bloğu size route etmesi gerekir, aksi halde trafik hiç gelmez.", label: 'Dış (Mapped) IP', type: 'text', validate: 'ip', optional: true, placeholder: '203.0.113.10', hint: 'Dışarıya görünen IP adresi' },
                        { name: 'static_inside', why: "Gerçek (real) adresin bulunduğu arayüzün nameif değeri. Object NAT <b>section 2</b>’de değerlendirilir; aynı hostu kapsayan bir manual/twice NAT (section 1) varsa o öncelikli olur ve bu kural hiç çalışmaz.", label: 'Inside Interface', type: 'text', optional: true, placeholder: 'inside', hint: 'İç taraf nameif' },
                        { name: 'static_outside', why: "Çevrilmiş adresin göründüğü arayüz. Yanlış seçilirse kural <code>show nat</code> çıktısında görünür ama hiç hit almaz — NAT sorunlarının en sık sebebi budur.", label: 'Outside Interface', type: 'text', optional: true, placeholder: 'outside', hint: 'Dış taraf nameif' }
                    ]
                }
            ],
            submit: 'Konfigürasyon Oluştur'
        ...
        cgFormBuilder(container, {
            ...
            sections: [
                ...
                },
                {
                    ...
                    showFor: ['radius'],
                    fields: [
                        { name: 'rad_server', why: "RADIUS sunucusu tarafında ASA’nın IP adresi <b>NAS client</b> olarak tanımlı değilse istekler cevapsız kalır ve ASA yalnızca timeout görür. Sunucuya giden yolun hangi arayüzden geçtiği de kritiktir.", label: 'RADIUS Sunucu IP', type: 'text', validate: 'ip', optional: true, placeholder: '10.0.0.200', hint: 'RADIUS sunucu IP adresi' },
                        { name: 'rad_key', why: "Shared secret uyuşmazsa RADIUS sunucusu isteği <b>sessizce yok sayar</b>; ASA tarafında bu timeout gibi görünür, yani hata mesajı yanıltıcıdır. İki tarafta birebir aynı olmalıdır.", label: 'Shared Secret', type: 'text', optional: true, placeholder: 'radius_secret', hint: 'RADIUS paylaşılan gizli anahtar' },
            ...
            sections: [
                ...
                },
                {
                    ...
                    fields: [
                        ...
                        { name: 'aa_active_ip', why: "Active/Active kurulumunda failover link IP’si; multiple context modu gerektirir ve tek context ile bu mod kullanılamaz.", label: 'Failover Link Aktif IP', type: 'text', validate: 'ip', optional: true, placeholder: '10.0.0.1', hint: 'Failover link IP' },
                        { name: 'aa_standby_ip', why: "Standby tarafın failover link adresi. Aynı subnette olmadığında gruplar hiç eşleşmez ve her iki cihaz kendi kararını verir.", label: 'Failover Link Standby IP', type: 'text', validate: 'ip', optional: true, placeholder: '10.0.0.2', hint: 'Failover link standby IP' },
                        { name: 'aa_mask', why: "Failover link maskesi; iki cihazda birebir aynı olmalıdır. Tutarsızlık, rol müzakeresinin hiç başlamamasına neden olur.", label: 'Subnet Mask', type: 'text', validate: 'subnet', optional: true, placeholder: '255.255.255.252', hint: 'Failover link maskesi' }
                    ]
                }
            ],
            submit: 'Konfigürasyon Oluştur'
            ...
            sections: [
                {
                    ...
                    showFor: ['network'],
                    fields: [
                        { name: 'net_obj_name', why: "Object adları ACL ve NAT satırlarında kullanıldığında yapılandırma okunabilir hale gelir; ancak kullanılan bir nesneyi silmek ona bağlı tüm satırları da kaldırır.", label: 'Network Object Adı', type: 'text', optional: true, placeholder: 'OBJ-WEB-SERVERS', hint: 'Tekil network nesnesi için isim' },
                        { name: 'net_subnet', why: "Nesnenin subnet değeri değiştirildiğinde ona referans veren <b>tüm</b> ACL ve NAT kuralları anında etkilenir. Tek satırlık bir düzenleme beklenmedik erişim açabilir.", label: 'Subnet (IP + Mask)', type: 'text', optional: true, placeholder: '10.1.2.0 255.255.255.0', hint: 'Nesne subnet değeri' },
                        { name: 'net_grp_name', why: "Object-group kullanmak ACL satır sayısını azaltır ama ASA bunları arka planda tek tek genişletir; aşırı büyük gruplar kural sayısını ve bellek kullanımını hızla artırır.", label: 'Object Group Adı', type: 'text', optional: true, placeholder: 'GRP-SERVERS', hint: 'Object group için isim' },
                ...
                },
                {
                    ...
                    showFor: ['service'],
                    fields: [
                        { name: 'svc_grp_name', why: "Servis grubu, aynı port kümesini birden çok kuralda tekrar yazmayı önler. Gruptan bir port çıkarmak ise o portu kullanan tüm kuralları aynı anda etkiler.", label: 'Service Group Adı', type: 'text', optional: true, placeholder: 'GRP-WEB-PORTS', hint: 'Port grubu için isim' },
                        ...
                        { name: 'svc_ports', why: "Portlar boşlukla ayrılır ve <code>www</code> gibi isimler kullanılabilir. Gereğinden fazla port eklemek saldırı yüzeyini sessizce genişletir; listeyi düzenli gözden geçirin.", label: 'Port Listesi', type: 'text', optional: true, placeholder: 'www https 8080', hint: 'Boşlukla ayrılmış port adı/numaraları' }
                    ]
                }
            ],
            submit: 'Konfigürasyon Oluştur'
