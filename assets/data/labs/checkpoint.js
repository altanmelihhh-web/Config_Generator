...
(function () {
    ...
    },
    {
        ...
        lesson: L('ClusterXL High Availability modunda bir üye <b>ACTIVE</b> (trafiği taşır), diğeri <b>STANDBY</b> (yedekte) bekler. Üyeler durumlarını CCP ile paylaşır; bağlantı tablosu <b>sync</b> arayüzü üzerinden eşitlenir, böylece failover\'da açık oturumlar kopmaz. Bir kritik aygıt (pnote: arayüz, fwd, ADMIN…) sorun bildirirse üye DOWN olur ve STANDBY üye devralır. <code>cphaprob stat</code> üyeleri, yükleri ve durumları; <code>cphaprob -a if</code> izlenen arayüzleri, sync\'i ve sanal IP\'leri gösterir; <code>clusterXL_admin down</code> / <code>up</code> kontrollü failover yapar.',
            ...
            'cphaprob stat\ncphaprob -a if\nclusterXL_admin down           # kalıcı olması gerekirse: clusterXL_admin down -p\ncphaprob stat                  # local DOWN, diğer üye ACTIVE, pnote ADMIN\nclusterXL_admin up\n\n# clish karşılıkları\nshow cluster state\nset cluster member admin down',
        ...
        tasks: [
            ...
            { t: 'Bakım için bu üyeyi yönetimsel olarak devre dışı bırakın (kontrollü failover).',
              ...
              fb: s => (s.ev.list().some(e => e.cladmin === 'down' && /\s-p\b/.test(e.raw || '')) ? 'Çalışır ama -p ile kalıcı yaptınız: üye yeniden başlatmadan sonra da DOWN kalır. Tek seferlik bakımda -p gerekmez; kullandıysanız geri alırken de clusterXL_admin up -p yazın.' : null) },
        ...
        solution: ['expert', PW, 'cphaprob stat', { answer: 1, v: 'active' }, 'cphaprob -a if', { answer: 3, v: 'eth3' }, 'clusterXL_admin down', 'cphaprob stat', 'clusterXL_admin up', { answer: 7, v: 'maintain' }, { answer: 8, v: 'attn' }, 'exit'],
